    ...
    */
        function _createWidget(widgetItem, queue) {
            ...
            if (state < Enum.WidgetState.INITIALIZED) {
                ...
                fileManager.loadJSFiles(widgetPath, (brease.config.editMode === true || brease.config.mocked !== true) && brease.config.testMode !== true).done(function (WidgetClass) {
        ...
        function _addToQueue(node, queue, contentId) {
            ...
            var widgetState = _widgetsController.getState(node.id);
            ...
            // widget does not exist or had an unsuccessful creation process before => add to model and to queue
                _widgetsController.addWithState(node.id, Enum.WidgetState.IN_PARSE_QUEUE);
