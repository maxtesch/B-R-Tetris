...
<xsl:stylesheet version="1.0"
  ...
  <!-- setParentContent: flag for compound widget content transformation: inner widgets get their parentContentId later from the compoundWidget instance -->
  ...
  <xsl:template match="/">
    ...
    <!--transform all widget options and wrap them in JS function calls in a script tag-->
    ...
    <xsl:element name="{$elemName}">
      ...
      <xsl:if test="$loadContent='true'">
        ...
        <xsl:for-each select="$content//script">
          <xsl:element name="script">
            <xsl:value-of select="scriptEx:replaceId(string(./text()),$idPrefix)" />
          </xsl:element>
        </xsl:for-each>
    ...
    <xsl:if test="$elName!='script'">
      <xsl:element name="{$elName}">
        <xsl:attribute name="id">
          <xsl:value-of select="scriptEx:replaceId(string($id),$idPrefix)" />
        </xsl:attribute>
        <xsl:for-each select="@*">
          <xsl:if test="name()!='id'">
