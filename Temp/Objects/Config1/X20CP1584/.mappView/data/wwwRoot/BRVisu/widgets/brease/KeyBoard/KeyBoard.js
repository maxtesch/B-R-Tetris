define(['widgets/brease/Window/Window', 'brease/events/BreaseEvent', 'brease/core/Utils', 'brease/controller/PopUpManager', 'widgets/brease/KeyBoard/libs/LayoutSelector', 'widgets/brease/KeyBoard/libs/AboutDisplay'],
    function (SuperClass, BreaseEvent, Utils, popupManager, LayoutSelector, AboutDisplay) {
        ...
        */
        ...
        function _addEventListeners() {
            var eventName = _getEventConfig(brease.config.virtualKeyboards);
            this.clear.on(eventName, this._bind('_onClear'));
            this.el.on(BreaseEvent.MOUSE_DOWN, 'button', this._bind(_onButtonMouseDown));
            this.el.on(eventName, 'button[data-value]', this._bind('_onValueButtonClick'));
            this.el.on(eventName, 'button[data-action]', this._bind('_onActionButtonClick'));
        ...
        function _removeEventListeners() {
            ...
            this.el.off(BreaseEvent.MOUSE_DOWN, 'button[data-value]', this._bind('_onValueButtonClick'));
            this.el.off(BreaseEvent.MOUSE_DOWN, 'button[data-action]', this._bind('_onActionButtonClick'));
            this.el.off(BreaseEvent.CLICK, 'button[data-value]', this._bind('_onValueButtonClick'));
            this.el.off(BreaseEvent.CLICK, 'button[data-action]', this._bind('_onActionButtonClick'));
            this.output.off();
        ...
        function _selectLayout(lang) {
            ...
            if (path === undefined) {
                ...
                widget.selector.setCurrentLayout(lang);
            }

            if (path !== undefined && path !== this.settings.html) {
                this.settings.html = path;
                this.instanceSettings.html = path;
                widget.options.value = widget.value;
                widget.options.text = widget.value;
                this.instanceSettings.value = widget.value;
                this.instanceSettings.text = widget.value;
                _removeEventListeners.call(widget);
                //widget.el.hide();
                widget.el.children(':not(.keyBoardHeader)').remove();
                if (this.plugin !== undefined) {
                    this.plugin.dispose();
                }
                require(['text!' + widget.settings.html], function (html) {
                    html = widget.postProcessHTML(html);
                    widget.el.append($(html).children(':not(.keyBoardHeader)'));
                    widget.el.attr('data-lang', $(html).attr('data-lang'));
                    widget.getElements();
                    widget.aboutDisplay = new AboutDisplay();
                    widget.aboutDisplay.init(widget.el);
                    if (widget.eventsAttached !== true) {
                        _addEventListeners.call(widget);
                    }
                    if (brease.config.detection.mobile === false) {
                        widget.output[0].removeAttribute('readonly');
                    }
                    //widget.el.show();
                    widget.setValue(widget.value);
                    widget.loadPlugin();
                    widget.setAlternativeLayouts(widget.settings.alternativeLayouts);
