...
module.exports = function (grunt) {
    ...
    */
        ...
        _xmlConvert.xml2js(coWiXML, {
            ...
            if (xmlObj) {
                ...
                var compoundXML = xmlObj['CompoundWidget'], 
                    ...
                    compoundWidget = _widgetObject(compoundXML, targetFolder, libraryName), 
                    // ancestor (system.widgets.CompoundWidget) widget info (path, name, etc)
                    ancestorWidget = _ancestorObject(corePath), 
                ...
                var widgetPathMappingFile = _modulePath.resolve(compoundWidget.dir, 'content/widgetPathMapping.xml');
                ...
                if (debug) {
                    _writeFile(_modulePath.resolve('/Temp/mvLog/compoundWidget.json'), JSON.stringify(compoundWidget));
                ...
                var baseCompoundWidget = grunt.file.readJSON(_modulePath.resolve(ancestorWidget.metaDir, ancestorWidget.name + '.json'));

                // json of compoundWidget as result of baseCompoundWidget patched with additional info and not as result of a widget compiler
                var widgetInfo = patchCoWi.run(baseCompoundWidget, compoundWidget, childInfos, Utils, DataTypes, grunt, debug);

                _writeFile(compoundWidget.metaClassPath + '.json', JSON.stringify(widgetInfo));

                // widget html file
                var templateHTML = grunt.file.read(_modulePath.resolve(ancestorWidget.dir, ancestorWidget.name + '.html')),
                    widgetHTML = _createWidgetHTML(templateHTML, ancestorWidget, compoundWidget);
                _writeFile(_modulePath.resolve(compoundWidget.dir, compoundWidget.name + '.html'), widgetHTML);

                // widget js file
                var templateJS = grunt.file.read(grunt.config('basePath') + '/templates/CompoundTemplate.js'),
                    widgetJS = cowiJsPrepare.createWidgetJS(templateJS, compoundWidget, childWidgetsList, Utils, DataTypes);
                _writeFile(_modulePath.resolve(compoundWidget.dir, compoundWidget.name + '.js'), widgetJS);

                // widget style file
                var templateStyle = grunt.file.read(grunt.config('basePath') + '/templates/CompoundTemplate.style'),
                    widgetStyle = _createWidgetStyle(templateStyle, compoundWidget, compoundWidget.commonProps);
                _writeFile(compoundWidget.metaClassPath + '.style', widgetStyle);

                // meta infos for use in JS
                var classInfo = jsPrepare.run(widgetInfo, compoundWidget.qualifiedName, ancestorWidget.type, false, { isCompound: true });
                _writeFile(_modulePath.resolve(compoundWidget.dir, 'designer/ClassInfo.js'), classInfo);
                ...
                _writeFile(compoundWidget.metaClassPath + '.xsd', widgetXsd);

                //styles of the ancestor widget as an JS object extracted from .widget file {styleProperties.StyleProperty:[],propertyGroups}                
                var superStyleObj = styleParser.parseFile(ancestorWidget.dir + '/meta/' + ancestorWidget.name + '.widget', grunt);
                ...
                var stylablePropsXML = _createStyleXML(superStyleObj, compoundWidget);
                ...
                var widgetXMLFile = compoundWidget.metaClassPath + '.widget',
                ...
                if (stylesXsd !== undefined && stylesXsd !== '') {
                    _writeFile(compoundWidget.metaClassPath + '_Styles.xsd', stylesXsd);
                } else {
                    _deleteFile(compoundWidget.metaClassPath + '_Styles.xsd');
                }
                ...
                if (eventActionXsd !== undefined && eventActionXsd !== '') {
                    _writeFile(compoundWidget.metaClassPath + '_EventsActions.xsd', eventActionXsd);
                } else {
                    _deleteFile(compoundWidget.metaClassPath + '_EventsActions.xsd');
                }

                // base and default scss files
                xsltTrans.transform(grunt, compoundWidget.metaClassPath + '_default.scss', grunt.config('basePath') + '/transformation/' + 'DefaultStyleTransformation.xsl', widgetXMLFile, [{
                    name: 'fileType',
                    value: 'scss'
                }]);
                xsltTrans.transform(grunt, compoundWidget.metaClassPath + '_base.scss', grunt.config('basePath') + '/transformation/' + 'DefaultStyleTransformation.xsl', widgetXMLFile, [{
                    name: 'fileType',
                    value: 'scss'
                }, {
                    name: 'createBase',
                ...
                grunt.config.set('sass.compoundWidget.src', _modulePath.resolve(compoundWidget.metaDir, compoundWidget.name + '_default.scss'));
                grunt.config.set('sass.compoundWidget.dest', _modulePath.resolve(compoundWidget.metaDir, compoundWidget.name + '_default.sass.css'));
                grunt.task.run(['sass:compoundWidget']);

                // create content
                var contentXML = _createContent(compoundXML, compoundWidget, childInfos);
                _writeFile(_modulePath.resolve(compoundWidget.dir, 'content/widgets.content'), contentXML);
                ...
                xsltTrans.transform(grunt, _modulePath.resolve(compoundWidget.dir, 'content/widgets.html'), _modulePath.resolve(xsltPath, 'content/HTMLBuilder.xsl'), _modulePath.resolve(compoundWidget.dir, 'content/widgets.content'), [{
                    name: 'basepath',
                ...
                xsltTrans.transform(grunt, _modulePath.resolve(compoundWidget.dir, 'content/widgets.scss'), _modulePath.resolve(xsltPath, 'content/StyleBuilder.xsl'), _modulePath.resolve(compoundWidget.dir, 'content/widgets.content'), [{
                    name: 'basepath',
                    value: _modulePath.resolve(grunt.config('basePath'), '../../').replace(/ /g, '%20').replace(/\\/g, '/') + '/'
                }, {
                    name: 'widgetFolders',
                ...
                // content css file
                grunt.config.set('sass.options.includePaths', [_modulePath.resolve(corePath + '/css/libs')]);
                grunt.config.set('sass.compoundContent.src', _modulePath.resolve(compoundWidget.dir, 'content/widgets.scss'));
                grunt.config.set('sass.compoundContent.dest', _modulePath.resolve(compoundWidget.dir, 'content/widgets.css'));
                grunt.task.run(['sass:compoundContent']);

                grunt.config.set('compoundWidget', compoundWidget);
    ...
    function _createContent(compoundXML, compoundWidget, childInfos) {
    
        if (Array.isArray(compoundXML.Widgets)) {
            _patchWidgets(compoundXML.Widgets, compoundWidget.library, childInfos);
        }
        var contentXML = _xmlConvert.js2xml({
            Content: { 
                $: {
                    id: compoundWidget.name,
                    width: compoundWidget.commonProps.width,
                    height: compoundWidget.commonProps.height,
    ...
    function _createWidgetHTML(templateHTML, ancestorWidget, compoundWidget) {

        return templateHTML.replace(ancestorWidget.qualifiedName, compoundWidget.qualifiedName);
    }

    function _createWidgetStyle(template, compoundWidget, defaults) {
        var newFile = template.replace('__WIDGET_NAME__', compoundWidget.type);
        newFile = newFile.replace('__WIDTH__', defaults.width);
        newFile = newFile.replace('__HEIGHT__', defaults.height);

        return newFile;
    }

    function _createStyleXML(superStyle, compoundWidget) {
        ...
        for (var i = 0; i < superStyle.styleProperties.StyleProperty.length; i += 1) {
            var prop = superStyle.styleProperties.StyleProperty[i]['$'],
                propName = prop['name'];
            if (compoundWidget.commonProps !== undefined && compoundWidget.commonProps[propName] !== undefined && ['top', 'left', 'zIndex'].indexOf(propName) === -1) {
                prop.default = compoundWidget.commonProps[propName];
                prop.owner = compoundWidget.type;
            }
        }
    ...
    function _widgetObject(compoundXML, ROOT, libraryName) {
        ...
        obj.customEvents = _convertEventActions(compoundXML['Events'], function (rawEvent) {
            var event = {};
            if (Array.isArray(rawEvent['Mappings'])) {
        ...
        obj.customActions = _convertEventActions(compoundXML['Actions'], function (rawAction) {
            var action = {};
            _extend(action, _convertActionArguments(rawAction['Arguments']));
    ...
    function _convertActionArguments(rawArgs) {
        var action = {};
        if (Array.isArray(rawArgs)) {
            action.arguments = {};
            if (rawArgs[0]['Argument']) {
                for (const rawArg of rawArgs[0]['Argument']) {
                    action.arguments[rawArg['$'].name] = {
                        name: rawArg['$'].name,
                        type: rawArg['$'].type,
    ...
    function _createWidgetFile(widgetInfo, json2xml, stylablePropsXML, propertyGroupsObj) {
        ...
        if (Array.isArray(widgetInfo.properties)) {
            var notStyleable = ['width', 'height', 'top', 'left', 'zIndex'];
            for (var i = widgetInfo.properties.length - 1; i >= 0; i -= 1) {
                var property = widgetInfo.properties[i];
                if (notStyleable.indexOf(property.name) !== -1) {
                    widgetInfo.properties.splice(i, 1);
                } else {
