define(['brease/events/EventHandler', 'brease/events/ClientSystemEvent', 'brease/events/BreaseEvent', 'brease/events/ServerEvent', 'brease/events/SystemGestures', 'brease/core/Utils', 'brease/events/VirtualEvents', 'brease/controller/objects/ContentStatus', 'brease/model/VisuModel'],
    function (EventHandler, ClientSystemEvent, BreaseEvent, ServerEvent, SystemGestures, Utils, VirtualEvents, ContentStatus, _visuModel) {
        /*jshint white:false*/
        'use strict';
        ...
        */
        var EventController = {
                ...
                init: function (runtimeService, bindingController, visuModel) {
                    ...
                    EventHandler.init(_runtimeService, bindingController);
                    _addListeners();
        ...
        function _addListeners(remove) {
            ...
            _initContentEvents(_runtimeService, remove);
            _initDialogEvents(_runtimeService, remove);
        ...
        function _initContentEvents(runtimeService, remove) {
            if (remove) {
                document.body.removeEventListener(BreaseEvent.FRAGMENT_SHOW, _contentLoadedHandler);
                if (runtimeService && runtimeService.removeEventListener) {
                    runtimeService.removeEventListener(ServerEvent.CONTENT_ACTIVATED, _contentActivatedHandler);
                }
                // revalidate events when new events are loaded
                document.body.removeEventListener(BreaseEvent.EVENTBINDING_LOADED, _eventbindingLoadedHandler);
        ...
        function _initDialogEvents(runtimeService, remove) {
            if (remove) {
                ...
                if (runtimeService && runtimeService.removeEventListener) {
                    runtimeService.removeEventListener(ServerEvent.CONTENT_ACTIVATED, _dialogContentLoadedHandler);
                }
                ...
                if (runtimeService) {
                    runtimeService.addEventListener(ServerEvent.CONTENT_ACTIVATED, _dialogContentLoadedHandler);
                }
        ...
        function _contentActivatedHandler(e) {
            ...
            var contentId = e.detail.contentId;
            //console.log('_contentActivatedHandler:', contentId);
            if (_eventQueue.contains(contentId)) {
                _eventQueue.remove(contentId);
                var content = _visuModel.getContentById(contentId);
                //console.log('_contentActivatedHandler:', contentId,content, content !== undefined, ',state:' + ((content) ? content.state : ''));
