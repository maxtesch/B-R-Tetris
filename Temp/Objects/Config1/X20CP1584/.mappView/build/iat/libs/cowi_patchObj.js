...
(function () {
    ...
    var patchCoWi = {
        ...
        run: function (baseCompoundWidget, compoundWidget, childInfos, Utils, DataTypes, grunt, debug) {
            this.grunt = grunt;
            this.debug = debug;
            ...
            _patchPropertyInfo(widgetInfo.properties, compoundWidget, childInfos, Utils, DataTypes);
            _patchEventInfo(widgetInfo.events, compoundWidget, childInfos);
            _patchLocalPropertyEvents(widgetInfo.events, compoundWidget);
            _patchActionInfo.call(this, widgetInfo.methods, compoundWidget, childInfos);
            _patchLocalPropertyActions(widgetInfo.methods, compoundWidget, Utils);
        ...
        failWithWarning: function (message) {
            if (patchCoWi.debug) {
    ...
    function _patchPropertyInfo(widgetProperties, compoundWidget, childInfos, Utils, DataTypes) {
    ...
    function _patchEventInfo(widgetEvents, compoundWidget, childInfos) {
        for (var key in compoundWidget.customEvents) {    
            var event = compoundWidget.customEvents[key],
                ...
                mappedId = obj.mappings[0]['$'].widget,
    ...
    function _patchLocalPropertyActions(widgetActions, compoundWidget, Utils) {
        for (var key in compoundWidget.customProps) {
            var prop = compoundWidget.customProps[key];
    ...
    function _patchActionInfo(widgetActions, compoundWidget, childInfos) {
        for (var key in compoundWidget.customActions) {
            ...
            _patchActionParameterInfo.call(this, obj, action);
    ...
    function _patchEventBindingInfo(widgetInfo, compoundWidget, childInfos) {
        ...
        if (Array.isArray(widgetInfo.eventBindings) && Array.isArray(widgetInfo.eventBindings[0].EventBinding)) {
            widgetInfo.eventBindings[0].EventBinding.forEach(function (eventBinding) {
    ...
    function _parseAction(Action) {
        ...
        actions.push(Action);
        if (Action.Result) {
            actions = actions.concat(_findResultActions(Action.Result));
        }
        return actions;
    }

    function _findResultActions(Result) {
        var actions = [];
        Result[0].ResultHandler[0].Action.forEach(function (objAction) {
