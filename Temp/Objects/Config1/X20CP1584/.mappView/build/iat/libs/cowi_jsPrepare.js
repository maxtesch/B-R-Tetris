...
(function () {
    ...
    var jsPrepare = {
        ...
        createWidgetJS: function (templateJS, compoundWidget, childWidgetsList, Utils, DataTypes) {
            ...
            if (Object.keys(compoundWidget.customProps).length > 0) {
                for (var name in compoundWidget.customProps) {
                    ...
                    if (Array.isArray(property.accumulatedMappings)) {
                        mapping += ',' + ctrl.lf + ctrl.tab(3) + property.name + ': { ';
            ...
            for (var propName in compoundWidget.customProps) {
                ...
                if (prop.defaultValue !== undefined && !prop.required) {
                    var parsed = _parseDefaultValueForJS(prop.defaultValue, prop.type, Utils, DataTypes);
                    if (parsed !== undefined) {
                ...
                if (prop.xsiType === 'LocalProperty') {
                    ...
                    initialCalls += ctrl.lf + ctrl.tab(2) + 'this.setChildProps(\'' + prop.name + '\', this.settings[\'' + prop.name + '\']);'; 
                }
            }
    ...
    function _parseDefaultValueForJS(value, type, Utils, DataTypes) {
