define([
...
], function (SuperClass, BreaseEvent, Renderer) {
	...
	*/
    ...
    p.init = function () {
        ...
        var elemWidth = parseInt($('#' + this.elem.id).css('width'),10);
        var elemHeigth = parseInt($('#' + this.elem.id).css('height'),10);
    ...
    p.setTransform = function (transform) {
        ...
        if (transform) {
            $.when(widget.renderer.deferred).done(function () {
                ...
                widget.data = JSON.parse(transform.replace(/\'/g, '"'));
    ...
    */
    ...
    p._customClickHandler = function (e) {
        ...
        if (this.isDisabled) {
			...
			*/
            ...
            var disabledClickEv = this.createEvent("DisabledClick", { hasPermission: hasPermission, origin: origin });
    ...
    p._createSVGPointFromClick = function (widget, e) {
        var svgContainer = $('#' + widget.elem.id + ' svg.breasePaperSVGContainer')[0],
            ...
            widgetLeftBorder = parseInt(widget.el.css("border-left-width"), 10),
            widgetTopBorder = parseInt(widget.el.css("border-top-width"), 10);

        if (e.type === "vmousedown") {
            point.x = e.pageX - Math.round(widget.el.offset().left) - widgetLeftBorder;
            point.y = e.pageY - Math.round(widget.el.offset().top) - widgetTopBorder;

            if (point.x < 0) {
                point.x = 0;
            } else if (point.x > widget.elem.clientWidth) {
                point.x = widget.elem.clientWidth;
            }

            if (point.y < 0) {
                point.y = 0;
            } else if (point.y > widget.elem.clientHeight) {
                point.y = widget.elem.clientHeight;
            }
        }
