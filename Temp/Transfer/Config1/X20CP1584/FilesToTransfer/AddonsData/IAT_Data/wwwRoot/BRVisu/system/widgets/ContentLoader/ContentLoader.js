define(['brease/core/BaseWidget', 'brease/enum/Enum', 'brease/events/BreaseEvent', 'brease/controller/WidgetController', 'brease/controller/WidgetParser', 'brease/controller/libs/Utils'], function (SuperClass, Enum, BreaseEvent, widgetsController, parser, controllerUtils) {
    ...
    */
    ...
    p.suspend = function suspend() {
        //console.log('%c' + this.elem.id + '.suspend(' + this.settings.contentId + ')', 'color:#00dddd');
    ...
    p.flush = function flush() {
        //console.log('%c' + this.elem.id + '.flush(' + this.settings.contentId + ')', 'color:#cc00cc');
    ...
    function _contentReadyHandler() {
        ...
        if (this.step === 0) {
            ...
            if (widget.settings.preCache) {
                if (widget.elem) {
                    // Todo: add event name to enum
                    widget.dispatchEvent(new CustomEvent('preCachedContentReady', { detail: { url: widget.settings.url, contentId: widget.settings.contentId }, bubbles: true }));
                }
                return;
            }
    ...
    function _unload(contentId, url) {
        ...
        widgetsController.disposeInContent(this.elem, contentId);
    ...
    function _load() {
        ...
        if (this.bindingController.isContentActive(widget.settings.contentId)) {
